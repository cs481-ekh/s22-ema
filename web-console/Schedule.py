...
def add_reminder_once(date, time, selectedProject, tag):
    schedule.every().day.at(time).do(run_single_reminder_job, start_date=date, project=selectedProject, tag=tag).tag(tag)

    getback = schedule.get_jobs()
    print(getback)
...
def add_reminder_weekly(startDate, startTime, endDate, endTime, selectedProject, tag):
    ...
    if dayofWeek == 'Monday':
        schedule.every().monday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Tuesday':
        schedule.every().tuesday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Wednesday':
        schedule.every().wednesday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Thursday':
        schedule.every().thursday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Friday':
        schedule.every().friday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Saturday':
        schedule.every().saturday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    elif dayofWeek == 'Sunday':
        schedule.every().sunday.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    else:
...
def add_reminder_daily(startDate, startTime, endDate, endTime, selectedProject,tag):
    schedule.every().day.at(startTime).until(endDate+' '+endTime).do(run_standard_reminder_job, project = selectedProject).tag(tag)
    getback = schedule.get_jobs()
    print(getback)
...
#        loadedProject = firebase.get_project_document_data(project)
#        print(loadedProject)
